...
Thrown by `selectWithParams()`, `writeWithParams()` and `readWithParams()` when a `{name:Type}` placeholder in the SQL has no matching param. The check runs client-side, before any request is sent. See [Native Query Parameters](native-params#missing-parameters).
