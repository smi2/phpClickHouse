# Играем с кластером ClickHouse из PHP, и делаем миграции.




*Это все - техническая заметка и наброски для статьи - еще будет несколько  рерайтов и проорфографов править буквы - позже*
...
Мы не будем глубоко рассказывать и отвечать на вопросы, 
...
предполагая что читатель знаком с этим.... бла-бла
...
## Пример установки и конфигурации Clickhouse
...
( Давайте тут сунем отсылка к первой части - там расписано про PHPClickHouse драйвер наш )
 ...
 * not total_replicas < 2 ( зависит от используемого cluster )
...
future_parts:       количество кусков с данными, которые появятся в результате INSERT-ов или слияний, которых еще предстоит сделать
...
В драйвере ,не запрашивать последние 4 столбца , реализовано через :
...
проект находится в зачаточном состоянии и после последнего митапа - когда CH возьмет на себя раскатывания изменений по всем нодам -> проект остановился.
