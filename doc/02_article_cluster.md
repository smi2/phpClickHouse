...
Когда количество узлов становится большим, то управление кластером становится неудобным. В результате мы создали простой и достаточно функциональный инструмент для миграции DDL-запросов в ClickHouse-кластер. И работу с кластером продемонстрируем на примере.
...
Драйвер выполняет подключение к кластеру и отправляет ping-запросы на каждый узел, которые перечисленны в DNS-записи.
...
В завершение статьи мы хотели бы провести небольшой опрос. А именно узнать, что вы думаете насчёт нашего инструмента миграции DDL-запросов:

* Мне интересен ваш опыт, потому что сам решаю аналогичную задачу
